...
std::wstring Utf8ToUtf16(const std::string& utf8_string) {
    ...
    int target_length = ::MultiByteToWideChar(
        ...
        // FIX: Explicitly cast the size_t returned by length() to an int.
    ...
    int converted_length = ::MultiByteToWideChar(
        CP_UTF8, MB_ERR_INVALID_CHARS, utf8_string.c_str(),
        // FIX: Do the same cast here.
        static_cast<int>(utf8_string.length()), utf16_string.data(), target_length);
    if (converted_length == 0) {
        return std::wstring();
    }
    return utf16_string;
